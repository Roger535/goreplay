...
func logtoDB(message *TCPMessage) {
	...
	if !message.IsIncoming {
		...
		hostport := string(proto.Header(payload, []byte("Host")))
		index := strings.IndexAny(hostport, ":")
		...
		for i := 0; i <= 5; i++ {
			_, err:= DB.Exec(`INSERT http_log (hostname,host,port,path,method,args,request_body,start_time,used_time,http_code,resp_body) values (?,?,?,?,?,?,?,?,?,?,?)`,
		                     hostname, host, port, path, method, args, request_body, message.Start.Add(time.Hour * 8),used_time.Seconds()*1000,http_code,resp_body)
			if err == nil {
				break
			}
